...
class CSET_Flight_Piece(CSET_Data):
    ...
    def __init__(self, flight_name, start_time, end_time, add_GOES=True):
        ...
        if add_GOES:
            self.add_GOES_data()
        self.extended = False # a hack, sorry future me
...
#         self.add_GOES_data()
    ...
    def add_AVAPS_data(self):
        all_AVAPS_files = glob.glob(os.path.join(utils.dropsonde_dir, "*.nc"))
        all_AVAPS_dates = [utils.as_datetime(dt.datetime.strptime(os.path.basename(i), 'D%Y%m%d_%H%M%S_PQC.nc')) for i in all_AVAPS_files]
        AVAPS_files = [(d, f) for d, f in zip(all_AVAPS_dates, all_AVAPS_files) if d>self.start_time and d<self.end_time]
        self.AVAPS_profiles = {d:utils.get_data_from_dropsonde(f) for (d,f) in AVAPS_files}
