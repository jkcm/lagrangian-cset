...
import numpy as np
import os
from hsproject.plots import get_goes_from_flightfile
...
flight_files = glob.glob(os.path.join(flight_folder, 'RF*.nc'))  # [0:1]

lt = LoopTimer(len(flight_files) * 3)
for flight_filename in flight_files:
    flight = os.path.basename(flight_filename)[:4]
    for delta_index, label in pairs:
        print('Working on flight {}, {}'.format(flight, label))
        try:
            data, long_names, units = get_goes_from_flightfile(
                flight_filename, goes_folder,
                vis_varnames + ('visible_count',),
        ...
        for varname in (
            ...
            for p in percentile.keys():
                for j in range(len(time_axis)):
                    too_much_ice_cloud = np.sum(
                        (data['cloud_phase'][j, :] == 2) |
                        (data['cloud_top_height'][j, :] > 4.))/float(
                        data['cloud_phase'][j, :].size) > max_high_cloud_pct
                    if varname in cloud_only_varnames:
                        use_me = (
                    ...
                    if use_me.any() and not too_much_ice_cloud:
                        percentile[p].append(
                            np.nanpercentile(data[varname][j, :][
                                use_me], int(p)))
                    else:
